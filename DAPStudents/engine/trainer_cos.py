...
from coStudents.data.build import (
    build_detection_semisup_train_loader,
    build_detection_test_loader,
    build_detection_semisup_train_loader_two_crops,
)
from coStudents.data.dataset_mapper import DatasetMapperTwoCropSeparate
from coStudents.modeling.meta_arch.ts_ensemble import EnsembleTSModel
from coStudents.checkpoint.detection_checkpoint import DetectionTSCheckpointer
from coStudents.solver.build import build_lr_scheduler
from coStudents.evaluation import PascalVOCDetectionEvaluator, COCOEvaluator
from coStudents.data.transforms.augmentation_impl import GLT
import copy
...
class CoSTrainer(DefaultTrainer):
    def __init__(self, cfg):
        """
        Args:
            cfg (CfgNode):
        ...
        self.ext_data = []
        self.img_vals = {}
        self.enhance_with_night_prior = GLT()
        self.register_hooks(self.build_hooks())
        self.student_proporsal_rpn = None
        self.student_proporsal_roih = None
        self.thresholding_pseudo_label = []
        self.coopertaed_pseudo_label = []
        self.cls_threhold = 0.8
        self.iou_threhold = 0.8
    ...
    def threshold_bbox(self, proposal_bbox_inst, thres=0.7, proposal_type="roih"):
        if proposal_type == "roih":
            ...
            # print(proposal_bbox_inst.scores)
            ...
            for i in range(match_quality_matrix_diag_filter.shape[0]):
                for j in range(match_quality_matrix_diag_filter.shape[1]):
                    ...
                    if match_quality_matrix_diag_filter[i][j] == True and valid_map[i] == False:
                        if student_proposals.pred_classes[j] == proposal_bbox_inst.pred_classes[i] and \
                                proposal_bbox_inst.scores[i] >= 0.5:
                            valid_map[i] = True
    ...
    def remove_label(self, label_data):
        for label_datum in label_data:
            if "instances" in label_datum.keys():
                # label_datum['gt_instances'] = label_datum["instances"]
    ...
    def run_step_full_semisup(self):
        ...
        label_strong, label_weak, unlabled_strong, unlabled_weak = data
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            label_strong.extend(label_weak)
            record_dict, _, _, _ = self.model(label_strong, branch="supervised_stu")
            ...
            record_dict = {}
            all_label = label_strong + label_weak
            record_all_label_data, _, _, _ = self.model(all_label, branch="supervised_stu")
            record_dict.update(record_all_label_data)

            gt_unlabel = self.get_label(unlabled_weak)
            unlabel_data_weak = self.remove_label(unlabled_weak)


            with torch.no_grad():
                (_, proposals_rpn_unsup_weak, proposals_roih_unsup_weak, _) = self.model_teacher(unlabel_data_weak,
                                                                                       branch="unsup_data_weak")

            # print(proposals_roih_unsup_weak)
            cur_threshold = self.cfg.SEMISUPNET.BBOX_THRESHOLD
            iou_threshold = self.cfg.SEMISUPNET.IOU_THRESHOLD
            joint_proposal_dict = {}
            # print(proposals_roih_unsup_weak)

            pesudo_proposals_roih_unsup_weak, _, _ = self.process_pseudo_label(
                proposals_roih_unsup_weak, cur_threshold, iou_threshold, "roih", "thresholding"
            )
            joint_proposal_dict["proposals_pseudo_roih_threshold"] = pesudo_proposals_roih_unsup_weak
            # print(proposals_roih_unsup_weak)

            unlabel_data_weak = self.add_label(unlabel_data_weak, joint_proposal_dict["proposals_pseudo_roih_threshold"])
            unlabel_data_strong = self.add_label(unlabled_strong, joint_proposal_dict["proposals_pseudo_roih_threshold"])
            # print(pesudo_proposals_roih_unsup)
            # (pseudo_losses,
            #  proposals_into_roih,
            #  rpn_proxy,
            #  roi_proxy,
            #  pred_idx) = self.model(
            #     unlabel_data_strong, branch="unsupdata_proxy"
            # )
            # pesudo_proposals_roih_unsup_weak, _, filter_cls_confidence = self.process_pseudo_label(
            #     proposals_roih_unsup_weak, cur_threshold, iou_threshold, "roih", "cooperated", roi_proxy
            # )
            #
            ...
            (pseudo_losses,
             ...
             roi_stu,
             pred_idx) = self.model(
                unlabel_data_strong, branch="unsupdata_stu"
            )
            new_pseudo_losses = {key + "_pseudo": self.cfg.SEMISUPNET.UNSUP_LOSS_WEIGHT * value for key, value in pseudo_losses.items()}
            record_dict.update(new_pseudo_losses)

            # new_det_losses = {key + "_det": value for key, value in detector_losses.items()}
            # record_dict.update(new_det_losses)
            record_prototype_contra_data = {}
            record_text_data = {}
            all_data = []
            all_data.append(label_strong)
            all_data.append(unlabel_data_strong)
            # all_data = label_data_q
            pro_loss, contra_loss, text_loss = self.model(all_data, branch="prototype_layer")
            # print(contra_loss)
            # print(pro_loss)
            record_prototype_contra_data[
                "loss_prototype_contra"] = pro_loss * self.cfg.SEMISUPNET.PROTO_WEIGHT + contra_loss * self.cfg.SEMISUPNET.CONTRA_WEIGHT
            record_dict.update(record_prototype_contra_data)

            record_text_data["loss_text"] = text_loss
            record_dict.update(record_text_data)
            ...
            for key in record_dict.keys():
                if key.startswith("loss"):
                    if key == "loss_rpn_loc_pseudo":
                        ...
                        loss_dict[key] = record_dict[key]
                    elif (key == "loss_prototype_contra"):
                        loss_dict[key] = record_dict[key] * 1
                    elif (key == "loss_text"):
                        loss_dict[key] = record_dict[key] * 1
                    else:
                        loss_dict[key] = record_dict[key] * 1
            # print(loss_dict['loss_rpn_cls_pseudo'].values())
            losses = sum(loss_dict.values())
    ...
    def _update_teacher_model(self, keep_rate=0.9996):
        ...
        for key, value in self.model_teacher.state_dict().items():
            if key in student_model_dict.keys():
                ...
                state_key = key.replace(state_key, state_key + '.0')
                new_teacher_dict[key] = (
