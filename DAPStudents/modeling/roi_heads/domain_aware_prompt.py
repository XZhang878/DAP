...
class PromptLearner(nn.Module):
    ...
    def __init__(self, classnames, clip_model, ctx=7):
        super().__init__()
        ...
        prompts = [prompt_prefix + ' ' + name + ' ' + domain + '.' for domain in domain_templates for name in
                   classnames]
    ...
    def forward(self):
        ...
        ctx = torch.cat([ctx_di, ctx_ds], dim=2).reshape(self.n_dms * self.n_cls, self.n_ctx_di + self.n_ctx_ds,
                                                         ctx_dim)  # [n_dms, n_cls, 16, 512]-> [n_dms * n_cls, 16, 512]
        prompts = torch.cat([
            prefix,  # [n_cls, 1, 512]
        ...
        ctx_ema = torch.cat([ctx_di_ema, ctx_ds_ema], dim=2).reshape(self.n_dms * self.n_cls,
                                                                     self.n_ctx_di + self.n_ctx_ds,
                                                                     ctx_dim)  # [n_dms, n_cls, 16, 512]-> [n_dms * n_cls, 16, 512]
...
class DAPromptHead(nn.Module):
    ...
    def get_embedding(self):
        ...
        text_features, text_features_all = self.text_encoder(prompts, tokenized_prompts)
        return text_features.float(), text_features_all.float()
